...
const WaterbodyPage = ({ waterbody }: WaterBodyProps) => {
  return (
    <>
      <Search />
      <Main>
        <LocationHeading pretitle="Skvelé kempy neďaleko vodnej plochy" title={waterbody.name} />
...
export const getStaticProps: GetStaticProps<Props, ContextParams> = async (context) => {
  ...
  try {
    const response = await fetch(`${process.env.BACKEND_HOST}/api/waterbody/slug/${encodedSlug}`);
